...
const OfficeConfiguration_1 = require("./OfficeConfiguration");
const Commands_1 = require("./Commands");
const rl = readline_1.default.createInterface({
    input: process.stdin,
    output: process.stdout
});
const office = OfficeConfiguration_1.OfficeConfiguration.getInstance();
function mainMenu() {
    console.log(`
=========================================
        ...
        rl.question('Enter start time (HH:MM, 24hr format): ', (startTime) => {
            ...
            rl.question('Enter duration in minutes: ', (duration) => {
                ...
                try {
                    ...
                    if (room) {
                        const command = new Commands_1.BookRoomCommand(room, durationMinutes, startTime);
                        command.execute();
...
function cancelBooking() {
    rl.question('Enter the room number to cancel booking: ', (roomId) => {
        ...
        try {
            const room = office.getRoom(parseInt(roomId));
            if (room) {
                const command = new Commands_1.CancelRoomBookingCommand(room); // Corrected to pass only the room object
                command.execute();
            }
            else {
